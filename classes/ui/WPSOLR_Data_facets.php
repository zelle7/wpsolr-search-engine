...
class WPSOLR_Data_Facets {
		...
		if ( count( $facets_in_results ) && count( $facets_to_display ) ) {
			...
			foreach ( $facets_to_display as $facet_to_display_id ) {
				...
				if ( isset( $facets_in_results[ $facet_to_display_id ] ) && count( $facets_in_results[ $facet_to_display_id ] ) > 0 ) {
					...
					$items_hierarchy = array();
					self::build_hierarchies(
						$items_hierarchy,
						$facet_to_display_id,
						$facets_in_results[ $facet_to_display_id ],
	 ...
	 *
	 * @param $results
	 * @param $facet_to_display_id
	 * @param $items
	 * @param $facets_selected
	 */
	public static function build_hierarchies( &$results, $facet_to_display_id, $items, $facets_selected ) {
