...
WpSolrExtensions::require_once_wpsolr_extension( WpSolrExtensions::EXTENSION_TABLEPRESS, true );

$extension_options_name = WPSOLR_Option::OPTION_TABLEPRESS;
$settings_fields_name   = 'extension_tablepress_opt';

$options          = get_option( $extension_options_name, array(
	'is_extension_active' => '0',
) );
$is_plugin_active = WpSolrExtensions::is_plugin_active( WpSolrExtensions::EXTENSION_TABLEPRESS );

$plugin_name    = "TablePress";
$plugin_link    = "https://wordpress.org/plugins/tablepress/";
$plugin_version = "(>= 1.7)";
...
<div id="extension_groups-options" class="wdm-vertical-tabs-content">
	<form action="options.php" method="POST" id='extension_groups_settings_form'>
		...
		<div class='wrapper'>
			...
			<div class="wdm_row">
				<div class='col_left'>Index TablePress shortcodes content within their post body.
				</div>
				<div class='col_right'>
					<input type='checkbox'
					       name='<?php echo $extension_options_name; ?>[is_index_shortcodes]'
					       value='is_index_shortcodes'
						<?php checked( 'is_index_shortcodes', isset( $extension_options['is_index_shortcodes'] ) ? $extension_options['is_index_shortcodes'] : '' ); ?>>

					Warning! "Expand shortcodes" must be selected in wpsolr's indexed data options.
				</div>
				<div class="clear"></div>
			</div>
			...
			<div class='wdm_row'>
				<div class="submit">
					<?php if ( $license_manager->get_license_is_activated( OptionLicenses::LICENSE_PACKAGE_TABLEPRESS ) ) { ?>
						<div class="wpsolr_premium_block_class">
							<?php echo $license_manager->show_premium_link( OptionLicenses::LICENSE_PACKAGE_TABLEPRESS, OptionLicenses::TEXT_LICENSE_ACTIVATED, true, true ); ?>
						</div>
					...
					<?php } else { ?>
						<?php echo $license_manager->show_premium_link( OptionLicenses::LICENSE_PACKAGE_TABLEPRESS, 'Save Options', true, true ); ?>
						<br/>
					<?php } ?>
				</div>
			</div>
